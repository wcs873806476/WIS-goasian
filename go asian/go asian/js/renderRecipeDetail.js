    ...
    fetch("data/recipes.json")
        ...
        .then(data => {
            ...
            if (recipe) {
                ...
                loadYoutubeVideo(recipe.media.video_keywords, recipe);
            } else {
...
// 加载 YouTube 视频
function loadYoutubeVideo(query, recipe) {
    const apiKey = ""; // ← 输入你的 YouTube API Key
    const maxResults = 1;
    const titleText = `🎥 How to make ${recipe.name}`;
